(() => {
      ...
      for (let attempt = 1; attempt <= MAX_RETRY + 1; attempt++) {
        ...
        try {
          ...
          if (mediaUrl) {
            ...
            if (characterCreated || characterCardInfo) {
              updateTask(taskId, {
                status: 'done',
                type: 'character',
                message: `角色卡创建成功：@${characterCardInfo?.username || '角色'}`,
                meta: { display: characterCardInfo?.display_name || characterCardInfo?.username || '' },
                logTail: lastChunk,
                logFull: logBufferAttempt || lastChunk,
                progress: 100
              });
  ...
  const normalizeBatchType = (raw) => {
    ...
    if (v === 'single' || v === 'same_prompt_files' || v === 'multi_prompt' || v === 'storyboard') return v;
    return 'single';
  };
  ...
  const loadRoles = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`响应解析失败（可能被浏览器插件/拦截器改写）：${(e && e.message) || String(e)}`);
      }
      roles = Array.isArray(data) ? data : [];
    } catch (e) {
      roles = [];
      // 说人话 + 给操作建议
      showToast('角色卡加载失败：请确认“服务器地址”可访问，且你已在管理台登录（浏览器本地存在 adminToken），然后点击“刷新”。', 'warn', {
        title: '角色卡未加载'
      });
      log('角色卡获取失败，点击“刷新”重试');
    }
    renderRoles();
  };
