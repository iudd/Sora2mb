        ...
        syncCharacters=async()=>{const btn=$('characterSyncBtn');if(btn){btn.disabled=true;btn.textContent='同步中...'}try{showToast('正在从所有活跃Token同步角色...','info');const r=await apiRequest('/api/characters/sync',{method:'POST'});if(!r){if(btn){btn.disabled=false;btn.textContent='同步'}return}const d=await r.json();if(d.success){showToast(d.message||`同步成功，新增 ${d.synced_count} 个角色`,'success');await loadCharacters()}else{showToast('同步失败: '+(d.detail||d.message||'未知错误'),'error')}}catch(e){showToast('同步失败: '+e.message,'error')}finally{if(btn){btn.disabled=false;btn.textContent='同步'}}},
        initCharactersUi=()=>{if(charactersUiInited)return;charactersUiInited=true;loadCharacterUi();updateCharacterViewButtons();const search=$('characterSearch');const sort=$('characterSort');const btnGrid=$('characterViewGrid');const btnList=$('characterViewList');const btnRefresh=$('characterRefreshBtn');const btnSync=$('characterSyncBtn');const btnEmptyRefresh=$('characterEmptyRefreshBtn');const btnGoGenerate=$('characterGoGenerateBtn');const btnClear=$('characterSearchClear');const bulkCopy=$('characterBulkCopy');const bulkClear=$('characterBulkClear');if(search){search.value=characterUi.query||'';const syncClear=()=>{if(btnClear)btnClear.classList.toggle('hidden',!search.value)};syncClear();search.addEventListener('input',()=>{characterUi.query=search.value||'';saveCharacterUi();syncClear();renderCharacters()});search.addEventListener('keydown',(e)=>{if(e.key==='Escape'){search.value='';characterUi.query='';saveCharacterUi();syncClear();renderCharacters();search.blur();}});}
            ...
            if(btnSync)btnSync.addEventListener('click',()=>syncCharacters());
