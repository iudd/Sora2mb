...
class Database:
    ...
    async def check_and_migrate_db(self, config_dict: dict = None):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Ensure character_cards table exists (new feature)
            if not await self._table_exists(db, "character_cards"):
                await db.execute("""
                ...
                """)
                print("  ✓ Created table 'character_cards'")
            else:
                # Backfill new columns for existing table
                if not await self._column_exists(db, "character_cards", "description"):
                    try:
                        await db.execute("ALTER TABLE character_cards ADD COLUMN description TEXT")
                        print("  ✓ Added column 'description' to character_cards")
                    except Exception as e:
                        print(f"  ✗ Failed to add column 'description' to character_cards: {e}")
    ...
    async def create_character_card(self, card: CharacterCard) -> int:
        """Persist a character card"""
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("""
                INSERT INTO character_cards (token_id, username, display_name, description, character_id, cameo_id, avatar_path, source_video)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (card.token_id, card.username, card.display_name, card.description, card.character_id, card.cameo_id, card.avatar_path, card.source_video))
            await db.commit()
            return cursor.lastrowid

    async def list_character_cards(self, limit: int = 200) -> List[dict]:
        """List character cards (newest first)"""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("""
                SELECT * FROM character_cards ORDER BY created_at DESC LIMIT ?
            """, (limit,))
            rows = await cursor.fetchall()
            return [dict(row) for row in rows]
    ...
    async def get_character_cards_by_usernames(self, usernames: List[str], token_id: int = None) -> List[dict]:
        ...
        批量匹配时同时尝试 username 与 display_name，提升容错。
        """
        if not usernames:
            return []

        unique_names = list(dict.fromkeys([u for u in usernames if u]))
        if not unique_names:
            return []

        placeholders = ",".join(["?"] * len(unique_names))
        base = (
            f"SELECT * FROM character_cards "
            f"WHERE (username IN ({placeholders}) OR display_name IN ({placeholders}))"
        )
        params = unique_names + unique_names  # username + display_name
    ...
    async def update_character_card_display_name(self, card_id: int, display_name: str):
        """Update display_name for a character card"""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                "UPDATE character_cards SET display_name = ? WHERE id = ?",
                (display_name, card_id)
            )
            await db.commit()

    async def delete_character_card(self, card_id: int) -> Optional[str]:
        """Delete a character card, return avatar_path for cleanup"""
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT avatar_path FROM character_cards WHERE id = ?", (card_id,))
            row = await cursor.fetchone()
