...
class TokenManager:
    ...
    async def check_username_available(self, access_token: str, username: str) -> bool:
        ...
        async with AsyncSession() as session:
            ...
            if response.status_code == 200:
                ...
                print(f"📄 响应内容: {response.text[:500]}")
                return False, "not_found", "exception", "not_found"
    ...
    async def auto_refresh_expiring_token(self, token_id: int, force: bool = False) -> bool:
        """
        ...
        try:
            ...
            # 📍 Step 4: 检查是否需要刷新
            if hours_until_expiry > 24 and not force:
